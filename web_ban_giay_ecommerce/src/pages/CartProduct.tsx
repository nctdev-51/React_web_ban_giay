...
export default function CartProduct() {
  ...
  /**
   * useMemo:
   * Chỉ tính lại khi cartItems thay đổi
   * Giúp tối ưu hiệu năng hơn so với việc mỗi lần render đều reduce lại
   */
  ...
  /**
   * useCallback:
   * Ghi nhớ hàm xử lý để tránh tạo lại hàm mới mỗi lần render
   * size phải là number vì cartSlice của bạn đang khai báo:
   * PayloadAction<{ id: number; size: number }>
   */
                  ...
                  <div className="flex items-center justify-between mt-6">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center bg-[#f6f6f6] rounded-full px-2 py-1">
                        <button
                          onClick={() =>
                            handleDecrease(product.id, product.selectedSize)
                          }
                        ...
                        <button
                          onClick={() =>
                            handleIncrease(product.id, product.selectedSize)
                          }
                          className="w-8 h-8 flex items-center justify-center text-lg rounded-full hover:bg-gray-200 transition-colors"
                        >
                          +
                    ...
                    <button
                      onClick={() =>
                        handleRemove(product.id, product.selectedSize)
                      }
